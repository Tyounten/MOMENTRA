...
const Profile = () => {
  ...
  // Safely compute following state (ensure both sides are strings)
  const isFollowing = !!userProfile && !!user &&
    (userProfile.followers || []).some(f => f?.toString() === user?._id?.toString());

  const handleTabChange = (tab) => setActiveTab(tab);

  const displayedPost = activeTab === 'posts' ? userProfile?.posts : userProfile?.bookmarks;

  const applyLocalFollowPatch = (nextType) => {
    // Build a new userProfile with updated followers array and count
    if (!userProfile || !user) return;
    const meId = user._id.toString();
    let nextFollowers;
    if (nextType === 'followed') {
      // add only if not present
      const hasMe = (userProfile.followers || []).some(f => f?.toString() === meId);
      nextFollowers = hasMe ? userProfile.followers : [...(userProfile.followers || []), meId];
    } else {
      // remove me
      nextFollowers = (userProfile.followers || []).filter(f => f?.toString() !== meId);
    }
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      ...
      // If backend returns the final type, ensure we’re in sync (idempotent)
      if (data?.success && (data?.type === 'followed' || data?.type === 'unfollowed')) {
        applyLocalFollowPatch(data.type);
      }
    } catch (err) {
      // Roll back optimistic change if API failed
      applyLocalFollowPatch(isFollowing ? 'followed' : 'unfollowed');
